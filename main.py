...
def check_revision():
    ...
    updated = False  # track if changes happened

    for key, value in new_data.items():
        if key not in old_data:
            old_data[key] = value
            updated = True  # new problem added

        solve_date = datetime.strptime(
            old_data[key]["date"], "%Y-%m-%d"
        ).date()
        ...
        if days == 2:
            message = (
                ...
                "Solved 2 days ago.\n"
                "Try solving again without seeing your old code 💪"
            )
            send_whatsapp_message(message)

    if updated:  # only save if something changed
        save_db(old_data)
